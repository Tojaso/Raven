	...
	if ct then
		...
		for _, c in pairs(ct) do
			if IsOn(c) and c.name then
				if IsOn(c.setResult) then
					c.testResult = c.setResult
					local doToggle = false
					local ts = IsShiftKeyDown()
					local tc = IsControlKeyDown()
					local ta = IsAltKeyDown()
					if c.toggleShift then
						if c.toggleControl then
							if c.toggleAlt then doToggle = (ts and tc and ta) else doToggle = (ts and tc) end
						else
							if c.toggleAlt then doToggle = (ts and ta) else doToggle = ts end
						end
					else
						if c.toggleControl then
							if c.toggleAlt then doToggle = (tc and ta) else doToggle = tc end
						else
							if c.toggleAlt then doToggle = ta end
						end
					end
					if doToggle then c.testResult = not c.testResult end
				else
					c.testResult = CheckConditionTests(c)
				end
			end
		end
...
function MOD:GetConditionText(name)
	...
	if IsOn(c.setResult) then
		logicString = ""
		if c.setResult then resultString = L["Set Result True"] else resultString = L["Set Result False"] end
		local a = nil
		if c.toggleShift then
			if c.toggleControl then
				if c.toggleAlt then a = "SHIFT+CONTROL+ALT" else a = "SHIFT+CONTROL" end
			else
				if c.toggleAlt then a = "SHIFT+ALT" else a = "SHIFT" end
			end
		else
			if c.toggleControl then
				if c.toggleAlt then a = "CONTROL+ALT" else a = "CONTROL" end
			else
				if c.toggleAlt then a = "ALT" end
			end
		end
		if a then resultString = resultString .. L["Modifier Key String"](a) end
	end
